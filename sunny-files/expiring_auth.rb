module Sunnyside
  def self.expiring_auth
    ...
    #   that gets all the visits that has that particular client number that is also GREATER than the most_recent_auth's end date
      ...
      latest_authorization = auth_data.order(:end_date).all.last
      AuthReport.new(auth_data, latest_authorization).create_csv
    }
  end
  ...
  class ExpiringAuth
    ...
    def provider
      ...
      # (Invoice.where(service_number: client_id).all.last.provider || Invoice.where(service_number: service_id).all.last.provider) || ''
    end
  ...
  class AuthReport
    attr_reader :client_auth, :invoices, :visits, :latest_authorization

    def initialize(client, latest_authorization)
      @latest_authorization   = latest_authorization
      @client_auth            = client
      @visits                 = Visit.where(member_id: client.get(:client_number)).all.select { |visit| latest_authorization.end_date < visit.dos }
    end

    def create_csv
      if expired?
        CSV.open('auth_report', 'a+') { |row| row << ['EXPIRED', latest_authorization.client, latest_authorization.provider, latest_authorization.start_date, latest_authorization.end_date, latest_authorization.auth] }
        if visits.count > 0
          # puts visits
          visits.each { |visit| CSV.open('auth_report', 'a+') { |row| row << [''       , visit.dos, visit.service_code, visit.units, visit.amount] } }
        end
      else
        CSV.open('auth_report', 'a+') { |row| row << ['PENDING', latest_authorization.client, latest_authorization.provider, latest_authorization.start_date, latest_authorization.end_date, latest_authorization.auth] }
      end
    end

    def expired?
      latest_authorization.end_date < Date.today
    end
  end
end